...
class oiopub_tracker {
	...
	//init
	function __construct($oio_version='') {
	...
	function reports() {
		...
		foreach($items as $i) {
			...
			if($pay_date !== $now_date && $pay_day === $now_day) {
				...
				$message = oiopub_email_readable(__oio("Dear") . " " . $i->adv_name . ",||" . __oio("You can view your weekly stats report using the link below") . ":||" . $oiopub_set->plugin_url . "/stats.php?email=" . urlencode($i->adv_email) . "&rand=" . $i->rand_id . "&period=week||" . __oio("Thanks") . ",|" . $oiopub_set->site_url);
	...
	function mail_stats($pid) {
		...
		if($oiopub_set->tracker['enabled'] == 1) {
			$item = $oiopub_db->GetRow("SELECT adv_email,item_channel,item_status,rand_id FROM " . $oiopub_set->dbtable_purchases . " WHERE item_id='$pid'");
			if($item->item_channel != 1 && $item->item_channel != 4) {
				if($item->item_status == -1 || $item->item_status == 1 || $item->item_status == 3) {
					...
					$oiopub_mail_extras .= "\n\n";
					$oiopub_mail_extras .= $oiopub_set->plugin_url."/stats.php?email=" . urlencode($item->adv_email) . "&rand=" . $item->rand_id;
				}
			}
		}
	}
	...
	function log_click($id, $url='') {
		...
		if($item->item_model == "days") {
			...
			$referer = oiopub_var('ref', 'get', $referer);
		}
		//update url?
		if(($item->item_channel == 3 && $item->item_type != 4) || $item->item_channel == 5) {
			$url = $url ? $url : $item->item_page;
		} else {
			$url = $url ? $url : $item->item_url;
		}
		//tracker enabled?
		if($oiopub_set->tracker['enabled'] != 1) {
		...
		if($oiopub_db->GetOne("SELECT pid FROM " . $oiopub_set->dbtable_tracker_clicks . " WHERE pid='$id' AND ip='$ip' AND time > '" . ($time_now - $dupe_limit) . "'")) {
			$this->debug_message('click', 'Dupe click');
			return $url;
		}
		//insert into db?
	...
	function debug_message($type, $msg) {
		...
		$log = $date . ' - ' . $oiopub_set->client_ip . ' - ' . $type . ' - ' . $msg . "\n";
		//log it
