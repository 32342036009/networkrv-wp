...
//check spam var
function oiopub_spam_check($var='', $block_empty=0) {
	$client = oiopub_client();
	return $client->allow_var($var, $block_empty);
}
...
function oiopub_var($name, $type, $default='', $clean=1) {
	//format input
	$name = oiopub_clean($name);
	$type = '_' . trim(strtoupper($type), '_');
	//var exists?
	if(isset($GLOBALS[$type]) && isset($GLOBALS[$type][$name]) && $GLOBALS[$type][$name]) {
		$default = $GLOBALS[$type][$name];
	}
	//clean output?
	if($clean == 1) {
		$default = oiopub_clean($default);
	}
	//return
	return $default;
}
