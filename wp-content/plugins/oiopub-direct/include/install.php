...
class oiopub_install {
	...
	function install_options() {
		...
		oiopub_add_config('inline_ads', array( "enabled"=>0, "selection"=>2, "price"=>array(0), "duration"=>array(0), "width"=>250, "height"=>200, "rotator"=>1, "queue"=>0, "showposts"=>2, "position"=>"right", "reuse"=>0, "showfeed"=>0, "nofollow"=>1, "nfboost"=>0, "template"=>'title-date', "defnum"=>1, 'model'=>"days" ), 1);
		oiopub_add_config('inline_defaults', array( 'type' => array(), 'image' => array(), 'site' => array(), 'html' => array(), 'cats' => array(), 'geo1' => array(), 'geo2' => array() ));
		oiopub_add_config('inline_links', array( "enabled"=>0, "price"=>array(0), "duration"=>array(0), "max"=>3, "nofollow"=>1, "nfboost"=>0, 'model'=>"days" ), 1);		
	...
	function install_zone($type, $id) {
		if($type == "link") {
			oiopub_add_config('links_'.$id, array( 'enabled'=>1, 'title'=>"Zone $id", 'list'=>1, 'price'=>array(0), 'duration'=>array(0), 'cols'=>1, 'rows'=>5, 'width'=>0, 'height'=>0, 'desc_length'=>0, 'rotator'=>1, 'queue'=>0, 'nofollow'=>1, 'nfboost'=>0, 'def_text'=>"", 'advertise_here'=>1, 'def_num'=>1, 'def_method'=>0, 'link_exchange'=>"", 'cats'=>0, 'model'=>"days" ), 1);
			oiopub_add_config('links_'.$id.'_defaults', array( 'type' => array(), 'url' => array(), 'anchor' => array(), 'desc' => array(), 'cats' => array(), 'geo1' => array(), 'geo2' => array() ));
		}
		if($type == "banner") {
			oiopub_add_config('banners_'.$id, array( 'enabled'=>1, 'title'=>"Zone $id", 'price'=>array(0), 'duration'=>array(0), 'cols'=>2, 'rows'=>2, 'width'=>125, 'height'=>125, 'rotator'=>1, "queue"=>0, 'def_image'=>"", 'nofollow'=>1, 'nfboost'=>0, 'spacing'=>10, 'def_num'=>1, 'def_method'=>0, 'link_exchange'=>"", 'cats'=>0, 'model'=>"days" ), 1);
			oiopub_add_config('banners_'.$id.'_defaults', array( 'type' => array(), 'image' => array(), 'html' => array(), 'site' => array(), 'cats' => array(), 'geo1' => array(), 'geo2' => array() ));
		}
		if($type == "custom") {
	...
	function upgrade() {
		...
		//check version
		if($oiopub_version != "2.62" || !oiopub_is_admin()) {
