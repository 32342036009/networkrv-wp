	...
	function wordpress_set_extras() {
		...
		if(empty($oiopub_set->plugin_url)) {
			if(function_exists('plugins_url') && $url = plugins_url()) {
				...
				$is_ssl = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] && $_SERVER['HTTPS'] != 'off') || ($_SERVER['SERVER_PORT'] == 443);
				$oiopub_set->plugin_url = 'http' . ($is_ssl ? 's' : '') . '://' . oiopub_clean($_SERVER['HTTP_HOST']) . '/wp-content/plugins/' . $oiopub_set->folder_name;
			}
		}
	}
