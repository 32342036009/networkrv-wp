...
class oiopub_output {
	...
	function format_output_banner($ads) {
		...
		foreach($ads as $ad) {
			...
			if(isset($ad->item_empty) && $ad->item_empty == 1) {
				...
				if(!empty($ad->item_notes)) {
					...
					//check image url
					$image_url = $this->check_domain($ad->item_url);
					//create img tag
					$_content = oiopub_image_display($image_url, $url, $this->options['width'], $ad_height, 0, $ad->item_tooltip);
					//not a flash ad?
		...
		if($this->options['fluid']) {
			$output .= '<script>' . "\n";
			$output .= '(function(){for(var a=document.getElementById("oio-banner-' . $this->zone . '"),b=a,c,d=window.getComputedStyle;!a.clientWidth;){b=b.parentNode;if(!b||!d||1!==b.nodeType)break;c=d(b,null).getPropertyValue("float");if("left"==c||"right"==c){b.style.width="100%";b.style.maxWidth="' . $zone_width . 'px";if(b.clientWidth)break;}}})();' . "\n";
			$output .= '</script>' . "\n";
		}
		//return
		return $output;
	...
	function check_domain($url) {
		$find = '/' . $this->settings->folder_name . '/uploads/';
		//should check url?
		if(strpos($url, $find) !== false) {
			$url = explode($find, $url, 2);
			$url = $this->settings->plugin_url . '/uploads/' . $url[1];
		}
		//return
		return $url;
	}
