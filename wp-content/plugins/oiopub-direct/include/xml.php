...
/*
PHP CLASSES Package: http://www.phpclasses.org/browse/package/4484.html
*/
...
class oiopub_xml {
	...
	function array2object ($arg_array)
	{
		...
		if (is_array($arg_array)) {
			...
			if(!is_numeric($keys[0])) $tmp = new oiopub_xml_object;
			foreach ($keys as $key) {
	...
	function xml_load_file($file, $resulttype = 'object', $encoding = 'UTF-8')
	{
		...
		$this->evalCode="";
		$values="";
		$data = oiopub_file_contents($file);
		if (!$data)
