...
class Advanced_Ads_Pro_Module_Inject_Content {
		...
		if ( isset ( $args['placement']['type'] ) ) {
			switch( $args['placement']['type'] ){
				case 'post_content_random' :
					...
					$content .= '<script>'
						. 'var advads_content_p = jQuery("#'. $wrapper_id .'")' . $paragraphs_selector . ';'
				...
				case 'post_above_headline' :
					$content .= '<script>'
						. 'advads.move("#'. $wrapper_id .'", "h1", { method: "insertBefore" });';
					$content .= '</script>';
				break;
				...
				case 'custom_position' :
					...
					$content .= '<script>'
						. 'advads.move("#'. $wrapper_id .'", "'. $target .'", { '. implode( ', ', $options ) .' });';
					$content .= '</script>';
				break;
			}
		}
	...
	private function get_paragraph_selector( $args ) {
		 ...
		 *  which are not empty
		 */
		$paragraphs_selector = '.parent().find("p:not(table p)").filter(function(){return jQuery.trim(this.innerHTML)!==""})';
	...
	public function inject_loop_post( $post, $wp_query = null ) {
		if ( ! $wp_query instanceof WP_Query || is_admin() || $wp_query->is_singular() || ! $wp_query->in_the_loop || ! $wp_query->is_main_query() ){
			return;
		}
	...
	public function content_injection_node( $node, $tag, $before ){
		// prevent injection in WordPress caption
		if ( 'img' === $tag && preg_match( '/\bwp-caption\b/', $node->parentNode->getAttribute( 'class' ) ) ) {
