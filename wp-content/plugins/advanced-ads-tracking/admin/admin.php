...
class Advanced_Ads_Tracking_Admin {
    ...
    public function admin_print_scripts() {
        ...
        if ( 'admin.php' == $pagenow && isset( $_GET['page'] ) && 'advanced-ads-stats' == $_GET['page'] ) {
            ...
            var _dataTableLang = {
                processing: '<?php _e( 'processing...', 'advanced-ads-tracking' ); ?>',
                search: '<?php _e( 'search:', 'advanced-ads-tracking' ); ?>',
                lengthMenu: '<?php _e( 'show _MENU_ entries', 'advanced-ads-tracking' ); ?>',
                info: '<?php _e( 'showing _START_ to _END_ of _TOTAL_ entries', 'advanced-ads-tracking' ); ?>',
                infoEmpty: '<?php _e( 'no element to show', 'advanced-ads-tracking' ); ?>',
                infoFiltered: '<?php _e( 'filtered from _MAX_ total entries', 'advanced-ads-tracking' ); ?>',
                infoPostFix: '',
                loadingRecords: '<?php _e( 'Loading...', 'advanced-ads-tracking' ); ?>',
                zeroRecords: '<?php _e( 'no matching records found', 'advanced-ads-tracking' ); ?>',
                emptyTable: '<?php _e( 'no data available in table', 'advanced-ads-tracking' ); ?>',
                paginate: {
                    first: '<?php _e( 'first', 'advanced-ads-tracking' ); ?>',
                    previous: '<?php _e( 'previous', 'advanced-ads-tracking' ); ?>',
                    next: '<?php _e( 'next', 'advanced-ads-tracking' ); ?>',
                    last: '<?php _e( 'last', 'advanced-ads-tracking' ); ?>'
                },
                aria: {
                    sortAscending:  '<?php _e( ': activate to sort column ascending', 'advanced-ads-tracking' ); ?>',
                    sortDescending: '<?php _e( ': activate to sort column descending', 'advanced-ads-tracking' ); ?>'
                }
            };
            var _dateName = {
                shortMonths: [
    ...
    public function add_column( $columns ) {
        $columns['ad_stats'] = __( 'Statistics', 'advanced-ads-tracking' );
        return $columns;
    }
    ...
    */
    ...
    public function enqueue_admin_scripts( $hook ) {
		...
		wp_register_script(AAT_SLUG . '-admin-scripts', plugins_url('assets/js/script.js', __FILE__), array('jquery'), AAT_VERSION);
		wp_localize_script(AAT_SLUG . '-admin-scripts', 'advads_tracking_clickable_ad_types', Advanced_Ads_Tracking_Plugin::$types_using_click_tracking );
        ...
        if ( $this->stat_page_hook == $hook ) {
			...
			$media_locale = array(
				'selectFile' => __( 'Select file', 'advanced-ads-tracking' ),
				'button' => advads__( 'select' ),
				'invalidFileType' => __( 'invalid file type', 'advanced-ads-tracking' ),
			);
            ...
            $stats_translations = array(
                'statsPerDate' => __( 'Stats per date', 'advanced-ads-tracking' ),
                'statsPerAd' => __( 'Stats per ad', 'advanced-ads-tracking' ),
                'clicks' => __( 'clicks', 'advanced-ads-tracking' ),
                'clicksFor' => __( 'clicks for "%s"', 'advanced-ads-tracking' ),
                'Clicks' => __( 'Clicks', 'advanced-ads-tracking' ),
                'impressions' => __( 'impressions', 'advanced-ads-tracking' ),
                'impressionsFor' => __( 'impressions for "%s"', 'advanced-ads-tracking' ),
                'Impressions' => __( 'Impressions', 'advanced-ads-tracking' ),
                'prevDay' => __( 'previous day', 'advanced-ads-tracking' ),
                'nextDay' => __( 'next day', 'advanced-ads-tracking' ),
                'prevMonth' => __( 'previous month', 'advanced-ads-tracking' ),
                'nextMonth' => __( 'next month', 'advanced-ads-tracking' ),
                'prevYear' => __( 'previous year', 'advanced-ads-tracking' ),
                'nextYear' => __( 'next year', 'advanced-ads-tracking' ),
                'prev%dDays' => __( 'previous %d days', 'advanced-ads-tracking' ),
                'next%dDays' => __( 'next %d days', 'advanced-ads-tracking' ),
                'clicksFromTo' => __( 'clicks from %1$s to %2$s', 'advanced-ads-tracking' ),
                'imprFromTo' => __( 'impressions from %1$s to %2$s', 'advanced-ads-tracking' ),
                'noDataFor' => __( 'There is no data for %1$s to %2$s', 'advanced-ads-tracking' ),
                'ad' => __( 'ad', 'advanced-ads-tracking' ),
                'ctr' => __( 'ctr', 'advanced-ads-tracking' ),
                'deletedAds' => __( 'deleted ads', 'advanced-ads-tracking' ),
                'date' => __( 'date', 'advanced-ads-tracking' ),
                'aTob' => __( '%1$s to %2$s', 'advanced-ads-tracking' ),
                'total' => __( 'total', 'advanced-ads-tracking' ),
                'noRecords' => __( 'There is no record for this period :(', 'advanced-ads-tracking' ),
                'periodNotConsistent' => __( 'The period you have chosen is not consistent', 'advanced-ads-tracking' ),
                'customPeriodMissing' => __( 'Some fields are missing for the custom period', 'advanced-ads-tracking' ),
				'invalidRecord' => __( 'One or more invalid records have been found in the database', 'advanced-ads-tracking' ),
				'noFile' => __( 'no file selected', 'advanced-ads-tracking' ),
				'group' => __( 'group', 'advanced-ads-tracking' ),
            );
			...
			$stats_file_locale = array(
				'unknownError' => __( 'An unexpected error occurred.', 'advanced-ads-tracking' ),
				'statsFrom' => __( 'stats from %1$s to %2$s', 'advanced-ads-tracking' ),
                'periodNotConsistent' => __( 'The period you have chosen is not consistent', 'advanced-ads-tracking' ),
        ...
        if ( $screen->id == $this->settings_page_id ) {
            ...
            $tracking_locale = array(
                'serverFail' => __( 'The server failed to respond to your request. Link structure not available.', 'advanced-ads-tracking' ),
                'unknownError' => __( 'An unexpected error occurred. Link structure not available.', 'advanced-ads-tracking' ),
                'linkAvailable' => __( 'Link structure available.', 'advanced-ads-tracking' ),
                'emailSent' => __( 'email sent', 'advanced-ads-tracking' ),
                'emailNotSent' => __( 'email not sent. Please check your server configuration', 'advanced-ads-tracking' ),
            );
		...
		if ( current_user_can( advanced_ads_tracking_db_cap() ) && isset( $_GET['page'] ) && 'advads-tracking-db-page' == $_GET['page'] ) {
            ...
            $dbop_locale = array(
                'serverFail' => __( 'The server failed to respond to your request.', 'advanced-ads-tracking' ),
                'unknownError' => __( 'An unexpected error occurred.', 'advanced-ads-tracking' ),
                'resetNoAd' => __( 'Please choose an ad', 'advanced-ads-tracking' ),
                'resetConfirm' => __( 'Are you sure you want to reset the stats for', 'advanced-ads-tracking' ),
				'SQLFailure' => __( 'The plugin was not able to perform some requests on the database', 'advanced-ads-tracking' ),
				'optimizeFailure' => __( 'Data were compressed but the tracking tables can not be optimized automatically. Please ask the server&#39;s admin on how to proceed.', 'advanced-ads-tracking' ),
            );
    ...
    public function register_overview_page_widget(){
        ...
        $wp_meta_boxes['toplevel_page_advanced-ads']['side']['high']['advads_overview_addon_tracking']['callback'] = array( 'Advanced_Ads_Tracking_Admin', 'render_overview_widget' );
    }
    ...
    public static function render_overview_widget(){
        include AAT_BASE_PATH . 'admin/views/overview.php';
    }
	...
	public function add_meta_box() {
		add_meta_box(
			'tracking-ads-box', __( 'Stats', 'advanced-ads-tracking' ), array( $this, 'render_metabox' ), Advanced_Ads::POST_TYPE_SLUG, 'normal', 'low'
		);
	}
    ...
    public function render_metabox(){
		...
		if ( 'ga' === $this->plugin->get_tracking_method() ){
			$warnings[] = array(
				'text' => __( 'These features are not available with the Google Analytics tracking method.', 'advanced-ads-tracking' ),
			);
		}
    ...
    public function settings_init($hook) {
        ...
        add_settings_field(
            'tracking-everything',
            __('Track every ad?', 'advanced-ads-tracking'),
        ...
        add_settings_field(
         ...
         add_settings_field(
            'link-target',
            __('Open in same window', 'advanced-ads-tracking'),
    ...
    public function render_ad_tracking_options($ad, $types) {
        ...
        $params_display = ( 'adsense' != $ad->type )? '' : 'display:none; ';
    ...
    public function save_options($options = array(), $ad = 0) {
        ...
        $options['tracking']['link'] = isset($_POST['advanced_ad']['tracking']['link']) ? trim( $_POST['advanced_ad']['tracking']['link'] ) : '';
		 ...
		 *  email reports
		 */
		$options['tracking']['report-recip'] = isset( $_POST['advanced_ad']['tracking']['report-recip'] )? sanitize_email( $_POST['advanced_ad']['tracking']['report-recip'] ) : '';
    ...
    public function load_stats($args = array(), $table){
        ...
        if (isset($args['period'])) {
            ...
            $now = $util->get_timestamp( time() - $gmt_offset );
            ...
            switch ($args['period']) {
                case 'today' :
                    ...
                    break;
                case 'yesterday' :
                    $start = $util->get_timestamp( time() - DAY_IN_SECONDS - $gmt_offset );
                    $start -= $start % Advanced_Ads_Tracking_Util::MOD_HOUR;
                    $end = $today_start;
                    break;
                case 'last7days' :
                    // last seven full days // -TODO might do last or current week as well
                    $start = $util->get_timestamp( time() - WEEK_IN_SECONDS - $gmt_offset );
                    $start -= $start % Advanced_Ads_Tracking_Util::MOD_HOUR;
                    break;
                case 'thismonth' :
                ...
                case 'lastmonth' :
                    ...
                    $start = $util->get_timestamp( mktime(0, 0, 0, date("m") - 1, 1, date("Y")) - $gmt_offset );
                ...
                case 'lastyear' :
                    ...
                    $start = $util->get_timestamp( mktime(0, 0, 0, 1, 1, date('Y') - 1) - $gmt_offset );
    ...
    public function reset_stats($ad_id = 0) {
        ...
        if ( null === $affected_rows ) {
            ...
            return array(
				'status' => true ,
				'msg' => __( 'All impressions and clicks removed.', 'advanced-ads-tracking' )
			);
        } elseif ( ! $affected_rows ) {
            return array(
				'status' => false,
				'msg' => __( 'No stats removed', 'advanced-ads-tracking' )
			);
        }
		return array(
			'status' => true,
			'msg' => sprintf( __( 'Impressions and clicks for ad ID %d removed', 'advanced-ads-tracking' ), $ad_id ),
		);
    }
    ...
    public function render_settings_link_target_callback(){
        ...
        include AAT_BASE_PATH . 'admin/views/setting_target.php';
    }
