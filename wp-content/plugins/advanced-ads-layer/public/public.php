...
class Advanced_Ads_Layer {
	...
	*/
	...
	public function footer_scripts() {
		...
		if ( $this->fancybox_is_enabled ) {
			wp_enqueue_script( 'advanced-ads-layer-fancybox-js', AAPLDS_BASE_URL . 'public/assets/fancybox/jquery.fancybox-1.3.4.pack.js', array( 'jquery' ), '1.3.4', true );
	...
	*/
	...
	public function close_script( $wrapper_id, $options, $ad_id = '' ) {
		...
		if ( isset( $options['layer_placement']['close']['enabled'] ) && $options['layer_placement']['close']['enabled'] 
			...
			if ( isset( $options['layer_placement']['close']['timeout_enabled'] ) ) {
				$timeout = ! absint( $options['layer_placement']['close']['timeout'] ) ? null : absint( $options['layer_placement']['close']['timeout'] );
				$set_cookie_string .= 'advads.set_cookie("timeout_placement_' . $options['output']['placement_id'] . '", 1, '. $timeout .'); ';
			} 

			$content .= $this->build_close_popup_js( $set_cookie_string, $wrapper_id );
		}
		// close button for ad itself
			...
			if ( isset( $options['layer']['close']['timeout_enabled'] ) ) {
				$timeout = ! absint( $options['layer']['close']['timeout'] ) ? null : absint( $options['layer']['close']['timeout'] );
	 ...
	 */
	private function build_close_popup_js( $set_cookie_string, $wrapper_id ) {
		...
		if ( $this->fancybox_is_enabled ) {
			...
			$script .= "jQuery( '#{$wrapper_id} .advads-close-button' ).click( function() { "
			. "advads.close( '#{$wrapper_id}' ); "
