...
class Advanced_Ads_Layer_Admin {
	...
	public function register_overview_page_widget() {
		...
		$wp_meta_boxes['toplevel_page_advanced-ads']['side']['high']['advads_overview_addon_layer']['callback'][0] = 'Advanced_Ads_Layer_Admin';
		$wp_meta_boxes['toplevel_page_advanced-ads']['side']['high']['advads_overview_addon_layer']['callback'][1] = 'render_overview_widget';
	}
	...
	/**
	 * add layer placement to list of placements
	 *
	 * @since 1.2.4
	...
	public function layer_placement_content( $placement_slug, $placement ) {
		switch ( $placement['type'] ) {
			case 'layer' :
				...
				ob_start();
				?><input type="checkbox" name="<?php echo $option_name; ?>[background]" value="1" <?php checked( $background, 1 ); ?>/>
				<p class="description"><?php _e( 'Put a half-transparent overlay over the background of the page if this ad is displayed.', 'advanced-ads-layer' ); ?></p><?php
				$option_content = ob_get_clean();
